...
The site publishes a single route tree at the site root — no locale prefix, no counterpart projection.
...
- `section`: reuse an existing section when possible. If adding one, also place it in `sections` in [website/docs.ts](../../../website/docs.ts).
