...
- **Mounting a plugin requires a consumer-manifest dependency, and a green typecheck does not prove it.** A composition row names a package, but the row only resolves if that package is reachable through `healProfilesModuleFallback`'s BFS over `dependencies` + `peerDependencies` from `apps/cli/package.json` (`packages/boot/app-boot/src/profile.ts:223`), which **skips** what it cannot resolve rather than failing. So: a row in `packages/bundle/<b>/cordis.patch.yml` needs the package in `packages/bundle/<b>/package.json`; a row in `apps/cli/config/agent-presets/<p>/agent.cordis.yml` needs it in `apps/cli/package.json`. `tsconfig.host.json` references are a compile-time graph and prove nothing about runtime resolution — four shipped features were inert for exactly this reason ([audit and plan](docs/improvement-plan.md)). `pnpm run verify-cordis-config` is the gate. Full new-package checklist: [docs/cookbook/adding-a-package.md](docs/cookbook/adding-a-package.md).
- Active work and its ordering live in [docs/improvement-plan.md](docs/improvement-plan.md); it supersedes `docs/ide-roadmap.md`, whose baseline table is stale in both directions.
...
Docs accompany every code change: update affected README and JSDoc contracts together. [docs/AGENTS.md](docs/AGENTS.md) owns the documentation standard: current-state prose, one physical line per paragraph, one home per fact, and word budgets.
