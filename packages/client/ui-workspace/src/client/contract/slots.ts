...
export type WorkspaceBrowserInjected = DirectoryPickingInjected & {
  ...
  /**
   * Delete a Session's durable log entirely (not just hidden). Unaccounts it
   * from its Workspace and drops it from the archive set if present. Rejects
   * for the currently active session or an unknown session id.
   */
