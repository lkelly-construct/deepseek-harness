...
Initialization advertises `general.positionEncodings: ['utf-16']`, `workspace: { workspaceFolders: true, configuration: true }`, `textDocument.hover.contentFormat: ['markdown', 'plaintext']`, `linkSupport: true` for definition and implementation, pull `diagnostic` support, and pushed `publishDiagnostics`, with no dynamic registration. The server's returned capabilities are authoritative: an unsupported operation, or synchronization without transient open/close, fails the query. `diagnostics` is the exception: a server advertisement of `diagnosticProvider` makes the host pull `textDocument/diagnostic` (position omitted — the whole document is queried); absent that, the host buffers `textDocument/publishDiagnostics` per document URI and returns the freshest batch after `didOpen`, bounded by `killGraceMs` so a silent server yields an empty result instead of hanging. An omitted server `positionEncoding` defaults to `utf-16`; any other value is a protocol error. The client answers `workspace/configuration` from static config, accepts lifecycle bookkeeping requests, and rejects `workspace/applyEdit` — it never applies edits or runs commands. Navigation maps `Location` directly and `LocationLink` from `targetUri` + `targetSelectionRange`; hover normalization takes valid `MarkupContent.value`, preserves string `MarkedString`s, renders language-tagged values as fenced code, and joins arrays with one blank line; diagnostics normalize severity (1-4, defaulting an omitted severity to 1/error), stringify string or number `code`s, and reject missing messages or malformed ranges. Missing results, malformed ranges or positions, and malformed hover/diagnostics encodings fail as structured `LSP_MALFORMED_RESPONSE` errors.
