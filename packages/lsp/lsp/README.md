...
The **LSP capability seam**: an abstract `LspService` (`ctx.lsp`) defining WHAT semantic code querying the harness has — go to definition, find references, find implementations, hover, whole-document diagnostics — over language-server providers, without binding the model contract to local subprocesses.
...
| `@deepseek-ai/dsh-tool-typecheck` | Consumer+fallback: offline `tsc --noEmit` diagnostics, separate from the seam |

The seam exposes exactly five semantic operations — `goToDefinition`, `findReferences`, `goToImplementation`, `hover`, `diagnostics` — and no generic JSON-RPC escape hatch, so no protocol payload or unreviewed command/mutation reaches a provider through `ctx.lsp`.
...
`LspQueryRequest` (`operation`, `filePath`, `position`, `workspaceRoot`) — every field required, so no field needs implementation defaulting and there is no `resolve()` step. `position` stays required for every operation; a `diagnostics` provider ignores it (the whole document is checked). Positions and ranges are zero-based UTF-16, matching the protocol; the tool owns the one-based cursor convention. `findReferences` always includes declarations — providers enforce this internally, so callers get no flag. `LspQueryResult` is a CLOSED discriminated union: `{ kind: 'locations'; locations; resolvedWorkspaceUri }` for navigation, `{ kind: 'hover'; hover }` for hover (content or `null`), and `{ kind: 'diagnostics'; diagnostics; resolvedWorkspaceUri }` where `diagnostics` is `LspDiagnostic[]` (`severity` 1-4, optional string `code`, `message`, `range`) — consumers `switch` to exhaustiveness so a new arm breaks compilation until handled. `resolvedWorkspaceUri` is the provider's canonical workspace `file:` URI; callers relativize location URIs against it instead of applying host-platform path rules to the possibly symlinked request root. See `src/types.ts` for the full contracts and `src/index.ts` for the `LspError` codes, including `LSP_DISPOSED` and `LSP_MALFORMED_RESPONSE`.
...
- **Five operations only** — symbols and call hierarchy are deferred (they need different schemas); mutations (rename, code actions, formatting) require separate tools with preview, permission, and write-policy integration. Diagnostics freshness and accumulation rules live in the providers (see `dsh-lsp-stdio`).
