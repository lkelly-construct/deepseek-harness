...
/** Reject changes or removals of English-only entries sealed by a prior manifest. */
...
/** Validate the closed kind tree and implemented/archive headers for English-only notes. */
export function validateArchiveArtifacts(artifacts: ReadonlyMap<string, Buffer>): string[] {
  ...
  for (const [path, content] of artifacts) {
    ...
    const sourceBase = basename(match[2])
    errors.push(...validateHeader(path, content, sourceBase))
  }
  return errors
}

/** Preserve every sealed English path/hash and append hashes for newly archived artifacts. */
