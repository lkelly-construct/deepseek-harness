...
describe('docsPages routes', () => {
  it('publishes exactly one home page that redirects to the quick-start page', () => {
    const homes = docsPages.filter(page => page.sidebar === null)
    expect(homes.map(page => page.route)).toEqual(['index.md'])
    const [home] = homes
    if (home === undefined) throw new Error('unreachable: length checked above')
  ...
  it('publishes every route exactly once', () => {
  ...
  it('publishes every Cordis core API page under the reference sidebar', () => {
    ...
    for (const file of files) {
      ...
      expect(page?.source, file).toBe(`docs/cordis-api/${file}`)
      expect(page?.section, file).toBe('Cordis Core API')
    }
  })

  it('publishes Cordis inherited alongside the core API pages', () => {
    const page = docsPages.find(page => page.route === 'reference/cordis-api/inherited.md')
    expect(page?.source).toBe('docs/cordis-api/inherited.md')
  })

  it('gives the persistence catalog a deep outline', () => {
    const page = docsPages.find(page => page.route === 'reference/persistence-catalog.md')
    expect(page?.source).toBe('docs/persistence-catalog.md')
    expect(page?.outline).toBe('deep')
  })

  it('publishes every generated-reference route', () => {
    const routes = [
    ...
    for (const route of routes) {
      expect(docsPages.find(page => page.route === route), route).toBeDefined()
    }
  })
})
